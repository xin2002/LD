...
class MultiLDHead(GFLHead):
    ...
    def __init__(self,
                 ...
                 loss_CA_MKD=dict(
                     type='SinkhornDistanceOneHot', loss_weight=0.25, T=10,g=1.0),
                 k=1.0,
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
        ...
        if len(pos_inds) > 0:
            ...
            k=self.k
            #print(k)
            c_ld_teacher = torch.stack([self.loss_CA_MKD(
                soft_corners[teacher_ind],
                target_distribution,
                weight=None, #weight_targets[:, None].expand(-1, 4).reshape(-1),
                
                avg_factor=None #weight_targets[:, None].expand(-1, 4).reshape(-1).sum()
                ) for teacher_ind in range(num_teacher)], dim=0)
            if(1):
                mini_ant=1e-6
                ld_weight_per=1/(length_box+mini_ant)
                ld_weight_per*=1/((1/(length_box+mini_ant)).mean())
                c_ld_teacher=F.softmax(c_ld_teacher*ld_weight_per)
            else:
                c_ld_teacher=F.softmax(c_ld_teacher)
            ld_weight=(1-c_ld_teacher)/(num_teacher-1)
            #print(c_ld_teacher)
            '''
            if(torch.any(c_ld_teacher>1)):
                print(">1")
            elif(torch.any(c_ld_teacher<0)):
                print(c_ld_teacher)
                print(weight_targets[:, None].expand(-1, 4).reshape(-1).min())
            '''
            ...
            #ld_weight =(c_ld_teacher**k)/(c_ld_teacher**k).sum(dim=0)
            #ld_weight=F.softmax(c_ld_teacher*k)
            #print(ld_weight)
            kd_weight = (1.0 - F.softmax(loss_kd_teacher, dim=0)
                         ) / (num_teacher - 1)
            #print("kd",kd_weight)
            loss_ld = num_teacher*torch.mul(ld_weight, loss_ld).mean()
            loss_kd = num_teacher*torch.mul(kd_weight, loss_kd).mean()
